...
from partnn.io_cfg import keyspecs, nullstr
from partnn.io_cfg import results_dir, summary_dir
from partnn.io_utils import read_pklhdf, get_ioidx
from partnn.io_utils import read_pdymlhdf, bcast_data2ioidx
from partnn.io_utils import get_h5dtypes, pd_concat
from partnn.io_utils import save_h5data, get_h5du
from partnn.io_utils import get_mtimes, deep2hie, resio
...
def summarize(cfgtree, epstep, keys=None):
    """
    ...
    for fpidx in rio.rslv_fpidx(cfgtree):
        ...
        grp_dicts = []
        for grp_name, grp_agg, grp_cast in grp_specs:
            ioidx = get_ioidx(h5grp, grp_name)
            grp_types_ = list(
                set(v for k, v in dtypes.items() if k.startswith(grp_name))
            )
            assert len(grp_types_) == 1, dedent(f'''
                Many dtypes in "{grp_name}": {grp_types_}''')
            ...
            if grp_type in ("pd.cat", "pd.qnt"):
                ...
                vals_raw = pd.read_hdf(h5pth, key=f'{partstrid}/{grp_name}')
            elif grp_type in ("pd.pkl",):
                vals_raw = read_pklhdf(h5grp, key=f'{grp_name}/pkl')
            elif grp_type in ("pd.yml",):
                vals_raw = read_pdymlhdf(h5grp, key=f'{grp_name}/yml')
            elif grp_type in ("np.arr",):
                vals_raw = h5grp[grp_name]
            else:
                raise RuntimeError(f"grp_type={grp_type} undef.")

            # broadcasting the data to have the same number of rows as `ioidx`
            vals_bcast = bcast_data2ioidx(data=vals_raw, ioidx=ioidx, key=grp_name, 
                dtype=grp_type, fpidx=fpidx)

            # Casting the group to any required type
            if grp_cast is None:
                grp_typecast, vals = grp_type, vals_bcast
            elif grp_cast == 'pd.cat':
                assert grp_type in ('pd.pkl', 'pd.qnt', 'pd.yml'), dedent(f'''
                    Cannot cast a non-pandas type "{grp_type}" to pd.cat. 
                    Please revise the "{grp_name}" casting entry 
                    in "grp_specs".''')
                grp_typecast = 'pd.cat'
                vals = vals_bcast.astype("category")
            else:
                raise ValueError(f'casting to "{grp_cast}" not implmntd.')
        ...
        for (grp_name, grp_agg, grp_cast), grp_dict in zip(grp_specs, grp_dicts):
    ...
    for gdl_ in pgrpdicts:
        ...
        if grp_type in ("pd.cat", "pd.qnt", "pd.pkl", "pd.yml"):
    ...
    for grp_dict in partgrpdicts:
        ...
        if grp_type in ("pd.cat",):
            ...
            grp_dict["smry"] = sudf
        elif grp_type in ("pd.qnt", "pd.cat", "pd.yml", "pd.pkl"):
    ...
    concatdict = dict()
    for grp_i, (grp_name, grp_agg, grp_cast) in enumerate(grp_specs):
        # Example:
        #   grp_name = 'hp'
        #   grp_type = 'pd.cat'
        #   grp_agg =  'last'
...
if __name__ == "__main__":
    ...
    exps = [("02_adhoc/01_mlphist", "01_vaehist",  ""),
            ("02_adhoc/02_cnnhist", "01_vaehist",  ""),
            ("02_adhoc/04_klstudy", "02_klstudy",  ""),
            ("02_adhoc/05_klstudy", "03_klstudy",  ""),
            ("02_adhoc/06_klstudy", "04_klstudy",  ""),
            ("02_adhoc/07_klstudy", "04_klstudy",  ""),
            ("02_adhoc/08_klstudy", "05_klstudy",  ""),
            ("02_adhoc/09_klstudy", "06_klstudy",  ""),
            ("02_adhoc/10_pppoly",  "07_pppoly",   ""),
            ("02_adhoc/11_mlphist", "08_mlphist",  ""),
            ("02_adhoc/12_cnnhist", "09_cnnhist",  "")]
    ...
    for experiment, edf in expdf.groupby('experiment'):
        ...
        for key, fpidf in edf.groupby('smrykey'):
            ...
            if key == '':
                savedata.update(summarize(fpidxs, epstep=100))
            else:
                savedata[key] = summarize(fpidxs, epstep=100)
            print('')
