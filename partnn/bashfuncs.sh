...
get_nextfreersrcidx() {
  # usage: get_nextfreersrcidx NRSRCS PIDS RANKS ACTVPIDS

  # This function tells you if you have a free resource among a pool of them.
  # You can give it 
  #
  #   (1) a list of past pids and their respective ranks, and
  #
  #   (2) a list of active unfinished pids
  #
  # and it will tell you the first free rank. If no rank is free, it will return -1.

  # The number of (gpu) resources
  NRSRCS=$1
  # The comma-sep list of all background pids (active or finished)
  PIDS=$2
  # The comma-sep list of ranks associated with those background pids (active or finished)
  RANKS=$3
  # The comma-sep list of active pids in the background
  ACTVPIDS=$4

  PYTHON_CODE=$(cat <<END

# The number of (gpu) resources
args_nrsrcs = int("${NRSRCS}")
# The comma-sep list of all background pids (active or finished)
args_pids = "${PIDS}"
# The comma-sep list of ranks associated with those background pids (active or finished)
args_ranks = "${RANKS}"
# The comma-sep list of active pids in the background
args_actvpids = "${ACTVPIDS}"

pids_lst = args_pids.strip(',').split(',')
ranks_lst = args_ranks.strip(',').split(',')
actvpids_lst = args_actvpids.strip(',').split(',')

assert args_nrsrcs > 0
assert len(pids_lst) == len(ranks_lst)

# Creating a set of busy ranks
actv_ranks = set()
for rank, pid in zip(ranks_lst, pids_lst):
    if pid in actvpids_lst:
        actv_ranks.add(rank)

rank_free = -1
for rank_int in range(args_nrsrcs):
    rank = str(rank_int)
    if rank not in actv_ranks:
        rank_free = rank
        break

print(rank_free, flush=True)

END
)

  # running the python code
  NEXTFREEIDX="$(python3 -c "$PYTHON_CODE" 2> /dev/null)"

  # Returning the results
  if [ $? -eq 0 ]; then
    # No errors happend, so we will print the results
    echo "$NEXTFREEIDX"
  else
    # Some error happend.
    echo "-2"
